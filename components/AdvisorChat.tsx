...
const SendIcon: React.FC<{ className?: string }> = ({ className }) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className={className}>
        <path d="M3.105 2.289a.75.75 0 00-.826.95l1.414 4.949a.75.75 0 00.95.826L11.25 9.75l-7.407-2.116A.75.75 0 003.105 2.29zM4.795 14.512a.75.75 0 00-.95-.826l-1.414-4.949a.75.75 0 00.826.95l7.407 2.116-6.873-1.964z" />
    </svg>
);
...
const AdvisorChat: React.FC = () => {
    ...
    return (
        <div className="bg-base-200 rounded-xl shadow-lg flex flex-col h-[60vh] max-h-[700px]">
            <div className="p-4 border-b border-base-300">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                    <AiIcon className="w-6 h-6 text-brand-primary" />
            ...
            <div className="flex-1 p-4 overflow-y-auto space-y-4">
                {messages.map((msg, index) => (
                    <div key={index} className={`flex items-start gap-3 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
                        ...
                        <div className={`max-w-md p-3 rounded-lg ${msg.sender === 'ai' ? 'bg-base-300 text-content-100' : 'bg-brand-primary text-white'}`}>
            ...
            <div className="p-4 border-t border-base-300">
                <form onSubmit={handleSendMessage} className="flex items-center gap-3">
                    <input
                        ...
                        disabled={isLoading}
                    />
                    <button type="submit" disabled={isLoading || !input.trim()} className="bg-brand-primary text-white rounded-lg p-2 disabled:bg-base-300 disabled:cursor-not-allowed hover:bg-brand-secondary transition-colors">
